...
frequency data!!!
use chi square or fisher's 


Regression Model for BRCA2 & phenotype
```{r}
...
```
Insignificant; P-value = 0.07 (≥ 0.05) --> Don't reject Null Hypothesis
...
anovXX_BMX <- aov(yBMX2 ~ xBMX2, data=XX)
summary(anovXX_BMX)
```
Insignificant; P-value = 0.06 (≥ 0.05) --> Don't reject Null Hypothesis
...
anovXX_BMX_All <- aov(yforAll ~ xforAll, data=XXandXY_BMX)
summary(anovXX_BMX_All)

```
insignificant; p-value = 0.19 (>0.05) ; don't reject null hypothesis
...
anovBRCA <- aov(x_BRCA2 ~ y_BRCA2, data=BRCA2)
summary(anovBRCA)
...
ANOVA _ check if it's normally distributed!!
if non-parametric, use non-parametric test
...
cor.test(BRCA2Level, MBXLevel)

```
insignificant p-value = 0.63 (>0.05) ; do not reject null hypothesis of no correlation 
